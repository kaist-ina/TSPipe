...
from bert import bert_to_sequential
from distillbert import ditilbert_to_sequential
from distiller import Distiller, TSPipeDistiller
...
def main():
    ...
    # tspipe args
    parser.add_argument('--tspipe-enable', action='store_true', dest='tspipe', help='Enable tspipe')
    ...
    if args.tspipe:
        student = ditilbert_to_sequential(student)
    elif args.n_gpu > 0:
    ...
    if args.teacher_config:
        tea_architecture_config = teacher_config_class.from_pretrained(args.teacher_config)
        tea_architecture_config.output_hidden_states = True

        print("Loading architecture")
        print(tea_architecture_config)
        teacher = teacher_model_class(tea_architecture_config)
    else:
        teacher = teacher_model_class.from_pretrained(args.teacher_name, output_hidden_states=True)

    if args.tspipe:
        teacher = bert_to_sequential(teacher)
    elif args.n_gpu > 0:
    ...
    if args.tspipe:
        distillerClass = TSPipeDistiller
    else:
        distillerClass = Distiller

    distiller = distillerClass(
